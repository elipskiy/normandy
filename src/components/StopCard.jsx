...
export default function StopCard({ stop }) {
  ...
  return (
    <div
      className={`stop${isOptional && !expanded ? " stop--collapsed" : ""}`}
      data-type={stop.type}
      onClick={isOptional ? () => setExpanded((e) => !e) : undefined}
      style={isOptional ? { cursor: "pointer" } : undefined}
    >
      <div className="stop-dot" />

      <div className={`stop-head${isOptional ? " stop-head--optional" : ""}`}>
        {stop.time && <span className="stop-time">{stop.time}</span>}
        <span className="stop-name">{stop.name}</span>
        {isOptional && (
          <ChevronDown
            size={13}
            className={`stop-expand-icon${expanded ? " stop-expand-icon--open" : ""}`}
          />
        )}
      </div>

      {isOptional && !expanded ? (
        stop.place && <div className="stop-place stop-place--collapsed">{stop.place}</div>
      ) : (
        <>
