export const DAYS = [
  {
    day: "Пятница",
    label: "Рывок",
    color: "var(--stone)",
    bg: "var(--stone-light)",
    icon: "Moon",
    summary: "Амстердам → Амьен · ~4 ч",
    stops: [
      {
        time: "~18:00",
        name: "Выезд из Амстердама",
        type: "drive",
        desc: "После работы — на юг. 4 часа через Бельгию.",
      },
      {
        time: "~22:00",
        name: "Aire d'Amiens",
        type: "sleep",
        desc: "24/7, ~€16/ночь. Просто спать.",
        link: "https://maps.google.com/?cid=16819355683746007983",
        maps: "https://www.google.com/maps/search/?api=1&query=Aire+de+Camping-Car+Amiens",
        rating: "4.3",
        coordinates: [49.8906, 2.2985],
      },
    ],
  },
  {
    day: "Суббота",
    label: "Сидр + Этрета",
    color: "var(--amber-dark)",
    bg: "var(--amber-light)",
    icon: "Apple",
    summary: "Амьен → Пеи-д'Ож → Этрета · сыр, сидр, скалы",
    foodTip: "Обед в Beuvron-en-Auge — яблочный пирог на кальвадосе, горячее суфле, запечённый камамбер. Ужин в Этрета или Фекане: мули-фрит у моря, нормандский рыбный суп.",
    stops: [
      {
        time: "10:30",
        name: "Fromagerie La Dégusterie",
        type: "main",
        place: "Pont-l'Évêque",
        desc: "Дегустация 4 AOP сыров (Pont-l'Évêque, Livarot, Camembert, Neufchâtel) + сидр. Планш €14–19, воркшоп ~45 мин. Пн–Сб 9–19.",
        rating: "4.6",
        link: "http://www.ladegusterie.com/",
        maps: "https://www.google.com/maps/search/?api=1&query=Fromagerie+La+D%C3%A9gusterie+Pont-l%27%C3%89v%C3%AAque",
        phone: "+33 2 31 65 54 10",
        photo: "photos/camembert.jpg",
        coordinates: [49.2951, 0.1879],
      },
      {
        time: "11:30",
        name: "Domaine Dupont",
        type: "main",
        place: "Victot-Pontfol",
        desc: "6000 яблонь, 13 сортов. Попроси сидр из бочек от кальвадоса. Туры в 11:00 и 15:00. Коктейль-воркшоп €35 (июль–авг, от 5 чел).",
        rating: "4.7",
        link: "https://www.calvados-dupont.com/",
        maps: "https://www.google.com/maps/search/?api=1&query=Domaine+Dupont+Victot-Pontfol",
        booking: "Без брони (май–сентябрь)",
        coordinates: [49.2461, 0.1279],
      },
      {
        time: "~12:15",
        name: "Ferme Cidricole Desvoye",
        type: "optional",
        place: "Beaufour-Druval",
        desc: "Маленькая органическая ферма, старый пресс, без толп.",
        maps: "https://www.google.com/maps/search/?api=1&query=Ferme+Cidricole+Desvoye+Beaufour-Druval",
        coordinates: [49.2161, 0.1540],
      },
      {
        time: "~12:30",
        name: "Calvados Pierre Huet",
        type: "optional",
        place: "Cambremer",
        desc: "С 1865, 5-е поколение. AOP Pays d'Auge — их лучший. €3.50, тур ~1ч15. Пн–Сб 9–12:30, 14–18, вс 10–13, 15–18.",
        maps: "https://www.google.com/maps/search/?api=1&query=Calvados+Pierre+Huet+Cambremer",
        coordinates: [49.1601, 0.1308],
      },
      {
        time: "13:00",
        name: "Обед в Бёврон-ан-Ож",
        type: "main",
        place: "Beuvron-en-Auge",
        desc: "«Самая красивая деревня Франции». Фахверк, площадь, кафе. Камамбер роти — запечённый с мёдом и орехами.",
        maps: "https://www.google.com/maps/search/?api=1&query=Beuvron-en-Auge+France",
        photo: "photos/beuvron.jpg",
        coordinates: [49.1769, 0.0603],
      },
      {
        time: "14:30",
        name: "Этрета",
        type: "main",
        place: "1 ч от Бёврона",
        desc: "Меловые арки и скалы. Подняться на тропу над Falaise d'Aval — вид на арку и Иголку сверху. Les Jardins d'Étretat: топиарный сад на краю обрыва. €10–12, 10:30–18:30.",
        maps: "https://www.google.com/maps/search/?api=1&query=%C3%89tretat+France",
        coordinates: [49.7072, 0.2019],
      },
      {
        time: "14:30",
        name: "Via Ferrata Клеси",
        type: "optional",
        place: "La Cambronnerie (альтернатива Этрета — в другую сторону)",
        desc: "250 м над рекой Орн. Мосты, переправы, зиплайн 400 м (~70 км/ч). Via ferrata €15–17, комбо с зиплайном ~€35.",
        rating: "4.4",
        link: "https://vertig.fun/",
        maps: "https://www.google.com/maps/search/?api=1&query=Via+Ferrata+Cl%C3%A9cy",
        phone: "+33 6 66 79 75 86",
        booking: "Бронь рекомендуется!",
        hours: "Апр–май, сент: вых 10–18. Июль–авг: ежедн.",
        photo: "photos/clecy.jpg",
        coordinates: [48.9172, -0.4657],
      },
      {
        time: "17:30",
        name: "Отель, Этрета / Фекан",
        type: "sleep",
        desc: "Ночёвка у моря. Фекан — 20 мин от Этрета, красивый рыбацкий порт + аббатство.",
        maps: "https://www.google.com/maps/search/?api=1&query=hotels+Etretat+France",
        coordinates: [49.7072, 0.2019],
      },
      {
        time: "17:30",
        name: "Camping des Rochers des Parcs",
        type: "optional",
        place: "Le Vey (если выбрали Via Ferrata)",
        desc: "На берегу Орны, у виа ферраты. Каяк, тропы. Бар.",
        rating: "4.6",
        link: "https://www.camping-normandie-clecy.eu/",
        maps: "https://www.google.com/maps/search/?api=1&query=Camping+des+Rochers+des+Parcs+Le+Vey",
        phone: "+33 2 31 69 70 36",
        coordinates: [48.9127, -0.4592],
      },
    ],
    food: [
      {
        name: "Le Pavé d'Auge",
        place: "Beuvron-en-Auge",
        desc: "Гастро в зале крытого рынка. Шеф Adrien Haye, сезонное меню, винная карта 600 позиций. Заказывай soufflé chaud au Calvados. Забронируй.",
        vibe: "обед, гастро",
        price: "€49–95 (меню)",
        rating: "4.7",
        maps: "https://www.google.com/maps/search/?api=1&query=Le+Pav%C3%A9+d%27Auge+Beuvron-en-Auge",
        phone: "+33 2 31 79 26 71",
        coordinates: [49.1769, 0.0605],
      },
      {
        name: "La Colomb'auge",
        place: "Beuvron-en-Auge, площадь",
        desc: "Крепери в фахверковом доме. Галеты с камамбером, andouille, фермерский яблочный сок, сидр в болях. Без брони.",
        vibe: "обед на ходу",
        price: "€10–18",
        rating: "4.6",
        maps: "https://www.google.com/maps/search/?api=1&query=La+Colomb%27auge+Beuvron-en-Auge",
        phone: "+33 2 31 39 02 65",
        coordinates: [49.1768, 0.0602],
      },
      {
        name: "La Route des Crêpes",
        place: "La Pommeraye (10 мин от Клеси)",
        desc: "Культовая крепери — муж и жена, всё органическое и локальное. Галет magret+pomme+miel+chèvre. Мало столов — бронируй!",
        vibe: "ужин, культ",
        price: "€10–20",
        rating: "4.7",
        maps: "https://www.google.com/maps/search/?api=1&query=La+Route+des+Cr%C3%AApes+La+Pommeraye",
        phone: "+33 2 31 69 15 88",
        coordinates: [48.9471, -0.4879],
      },
      {
        name: "La pomme d'happy",
        place: "Voie Verte, у Клеси",
        desc: "Кантин на велодорожке. Plat du jour, стейк-фрит, крепы. Всё локальное, цены мизерные. Велосипедисты и местные. Без брони.",
        vibe: "ужин на ходу",
        price: "€10–20",
        rating: "4.8",
        maps: "https://www.google.com/maps/search/?api=1&query=La+pomme+d%27happy+Cl%C3%A9cy",
        phone: "+33 2 31 59 77 86",
        coordinates: [48.9195, -0.4737],
      },
    ],
    nearby: [
      {
        type: "photo",
        name: "Les Jardins d'Étretat",
        desc: "Топиарный сад прямо на краю скалы — скульптуры из стриженых кустов с видом на арку Aval и Иголку. Лучшая точка для фото всего Этрета. €10–12, 10:30–18:30.",
        light: "Полдень или за 1 ч до заката",
        maps: "https://www.google.com/maps/search/?api=1&query=Les+Jardins+d%27Etretat",
        coordinates: [49.7054, 0.1973],
      },
      {
        type: "photo",
        name: "Falaise d'Aval — тропа над аркой",
        desc: "Подъём ~20 мин от пляжа. Вид на все три арки и Иголку одновременно. Бесплатно.",
        light: "Золотой час",
        maps: "https://www.google.com/maps/search/?api=1&query=Falaise+d%27Aval+Etretat",
        coordinates: [49.7038, 0.2052],
      },
      {
        name: "Château de Crèvecœur",
        desc: "Средневековый замок — 10 мин от Камбремера.",
      },
      {
        type: "photo",
        name: "Vergers en fleurs — Route du Cidre",
        desc: "Середина апреля = пик цветения яблонь. Белые и розовые цветы над зелёным нормандским бокажем с фахверковыми фермами. Практически никто не фотографирует. Снимай с запада во второй половине дня.",
        light: "Вечер, против света",
        maps: "https://www.google.com/maps/search/?api=1&query=Route+du+Cidre+Cambremer",
        coordinates: [49.1601, 0.1308],
      },
      {
        type: "photo",
        name: "Pain de Sucre — меандр реки Орн",
        desc: "171 м над долиной. Река делает петлю внизу — классический изгиб. В сентябре–октябре утром туман стелется по долине, деревья торчат из белого. Парковка у Saint-Omer, 15 мин пешком.",
        light: "Рассвет, осенний туман",
        maps: "https://www.google.com/maps/search/?api=1&query=Pain+de+Sucre+Cl%C3%A9cy",
        coordinates: [48.9104, -0.4694],
      },
    ],
  },
  {
    day: "Воскресенье",
    label: "Мон-Сен-Мишель",
    color: "var(--blue)",
    bg: "var(--blue-light)",
    icon: "Landmark",
    summary: "Клеси → Мон-Сен-Мишель · ~2 ч + весь день",
    foodTip: "По дороге заедь в Villedieu-les-Poêles — там отличные гречневые блины. На ужин — ягнёнок с солёных лугов (pré-salé): овцы пасутся у залива, мясо получается нежным и чуть солоноватым. Спроси именно «AOP Prés-Salés du Mont-Saint-Michel».",
    stops: [
      {
        time: "08:30",
        name: "Roche d'Oëtre",
        type: "optional",
        place: "Saint-Philbert-sur-Orne (15 мин южнее Клеси)",
        desc: "118 м скала над ущельем Рувр. Тропы по краю обрыва, скальная формация в форме лица. Бесплатно, парковка для кемпера.",
        maps: "https://www.google.com/maps/search/?api=1&query=Roche+d%27O%C3%ABtre+Saint-Philbert-sur-Orne",
        coordinates: [48.8630, -0.4917],
      },
      {
        time: "09:30",
        name: "Выезд на MSM",
        type: "drive",
        desc: "~2 ч через бокаж. По дороге — Villedieu-les-Poêles (литейня колоколов XII века, медные мастерские).",
      },
      {
        time: "~11:00",
        name: "Авранш — Jardin des Plantes",
        type: "optional",
        place: "Avranches",
        desc: "По дороге. Панорама на залив и силуэт Mont. Бесплатно, 20 мин.",
        maps: "https://www.google.com/maps/search/?api=1&query=Jardin+des+Plantes+Avranches",
        coordinates: [48.6863, -1.3553],
      },
      {
        time: "12:00",
        name: "Мон-Сен-Мишель",
        type: "main",
        desc: "Аббатство (€16), стены, улочки. Шаттл бесплатный (7:30–00:00). Вечером — подсвеченный остров.",
        maps: "https://www.google.com/maps/search/?api=1&query=Mont+Saint-Michel+France",
        booking: "Приливы: maree.info",
        photo: "photos/msm.jpg",
        coordinates: [48.6361, -1.5115],
      },
      {
        time: "вечер",
        name: "Aire de Beauvoir",
        type: "sleep",
        desc: "€16.80/сутки + €1.30 налог. Нужна карта CAMPING-CAR PARK (€5 разово). Шаттл в 2.5 км, велодорожка до замка.",
        rating: "4.4",
        link: "https://www.campingcarpark.com/",
        maps: "https://www.google.com/maps/search/?api=1&query=Aire+de+Camping+Car+Beauvoir+Mont+Saint-Michel",
        phone: "+33 1 83 64 69 21",
        coordinates: [48.6152, -1.4993],
      },
    ],
    food: [
      {
        name: "La Flambée des Cuivres",
        place: "Villedieu-les-Poêles (по дороге)",
        desc: "Одна из лучших крепери региона. Галеты с andouille de Vire + камамбер, teurgoule на десерт. Без брони.",
        vibe: "обед на ходу",
        price: "€10–20",
        rating: "4.8",
        maps: "https://www.google.com/maps/search/?api=1&query=Creperie+La+Flamb%C3%A9e+des+Cuivres+Villedieu-les-Po%C3%AAles",
        coordinates: [48.8375, -1.2175],
      },
      {
        name: "La Casa de Quentin",
        place: "Beauvoir, 102 Rue Saint-Michel",
        desc: "2 400+ отзывов, рядом с MSM но не в туристической зоне. Agneau pré-salé, нормандская кухня. Забронируй на ужин.",
        vibe: "ужин",
        price: "€20–30",
        rating: "4.7",
        maps: "https://www.google.com/maps/search/?api=1&query=La+Casa+de+Quentin+Beauvoir",
        coordinates: [48.6117, -1.4959],
      },
      {
        name: "Le Relais Gascon",
        place: "Pontorson, 15 Rue de Tanis",
        desc: "Домашняя кухня, комфорт. 1 100+ отзывов, любимое место местных. Без брони.",
        vibe: "обед/ужин на ходу",
        price: "€20–30",
        rating: "4.6",
        maps: "https://www.google.com/maps/search/?api=1&query=Le+relais+gascon+Pontorson",
        coordinates: [48.5561, -1.5089],
      },
      {
        name: "Мер Пулар",
        place: "На Mont",
        desc: "Суфле-омлет. €30+ за яйца. Лучше посмотреть через окно.",
        vibe: "ловушка",
        price: "€€€",
        rating: "3.6",
        trap: true,
        maps: "https://www.google.com/maps/search/?api=1&query=La+M%C3%A8re+Poulard+Mont+Saint-Michel",
        coordinates: [48.6361, -1.5116],
      },
    ],
    nearby: [
      {
        type: "photo",
        name: "La Roche Torin — herbus с МСМ",
        desc: "ЭТО то место с зелёными кочками. Мыс чуть приподнят над prés-salés — МСМ по центру, каналы воды дают зеркало, овцы на среднем плане. Паркинг у деревни Courtils, 15 мин пешком. Не путай с отелем.",
        light: "Золотой час / закат (МСМ на юго-запад)",
        maps: "https://www.google.com/maps/search/?api=1&query=Pointe+de+la+Roche+Torin+Courtils",
        coordinates: [48.6286, -1.4511],
      },
      {
        type: "photo",
        name: "Pointe du Grouin du Sud",
        desc: "Дикий мыс Conservatoire du Littoral: скалы, никаких построек в кадре, МСМ под косым углом. Телевик сделает остров крупнее. Закат, смотришь на северо-запад.",
        light: "Закат",
        maps: "https://www.google.com/maps/search/?api=1&query=Pointe+du+Grouin+du+Sud+Manche",
        coordinates: [48.5822, -1.5536],
      },
      {
        type: "photo",
        name: "Bec d'Andaine — Genêts, отлив",
        desc: "12 км мокрого песка при большом отливе. МСМ отражается в зеркальном слое воды. Бочши (bouchots) дают ведущие линии к замку. Рассвет, смотришь на восток. Надевай сапоги — песок мягкий.",
        light: "Рассвет, 2–3 ч после полной воды",
        maps: "https://www.google.com/maps/search/?api=1&query=Bec+d%27Andaine+Gen%C3%AAts",
        coordinates: [48.6511, -1.5514],
      },
      {
        type: "photo",
        name: "Cabane Vauban, Champeaux",
        desc: "80 м клифа над заливом, МСМ на горизонте через 20+ км. Eisenhower назвал этот километр «самым красивым во Франции». Июль–август — вереск цветёт фиолетовым. Домик Людовика XIV как передний план.",
        light: "Вечер, телевик на МСМ",
        maps: "https://www.google.com/maps/search/?api=1&query=Champeaux+Manche+cabane+Vauban",
        coordinates: [48.8003, -1.5347],
      },
      {
        name: "Barrage du Couesnon",
        desc: "Дамба у MSM — вечерний вид на остров с уровня воды. Пешком от парковки, бесплатно.",
        maps: "https://www.google.com/maps/search/?api=1&query=Barrage+du+Couesnon+Mont-Saint-Michel",
        coordinates: [48.6286, -1.5139],
      },
      {
        name: "Fonderie Cornille-Havard",
        desc: "Литейня колоколов с XII века в Villedieu-les-Poêles. Смотришь как льют бронзу. ~€7, по дороге.",
        maps: "https://www.google.com/maps/search/?api=1&query=Fonderie+Cornille-Havard+Villedieu-les-Po%C3%AAles",
        coordinates: [48.8375, -1.2175],
      },
    ],
  },
  {
    day: "Понедельник",
    label: "Aéroplume + Котантен + Париж",
    color: "var(--violet)",
    bg: "var(--violet-light)",
    icon: "Wind",
    summary: "Экоссвиль → (Барфлёр) → Париж · полёт + мидии + кабаре",
    foodTip: "Обед в Барфлёре у порта — мули-фрит или устрицы прямо с лодок (если заезжаете). Вечером в Париже — ужин рядом с Crazy Horse на Avenue George V.",
    stops: [
      {
        time: "08:00",
        name: "Выезд из Бовуара",
        type: "drive",
        desc: "~2 ч до Экоссвиля. Выезжай не позже 08:00, чтобы успеть на 10:00.",
      },
      {
        time: "10:00",
        name: "Aéroplume",
        type: "main",
        place: "Экоссвиль",
        desc: "Полёт на гелиевом шаре внутри 150 м ангара. €75/чел, 30 мин. До 90 кг. Забронировано: 10:00 и 10:30.",
        rating: "4.5",
        link: "https://aeroplume.fr/",
        maps: "https://www.google.com/maps/search/?api=1&query=A%C3%A9roplume+%C3%89causseville",
        phone: "+33 6 41 98 32 97",
        booking: "Забронировано ✓",
        hours: "С 11 апреля. Ежедн. 9–17:30.",
        coordinates: [49.3785, -1.3978],
      },
      {
        time: "11:30",
        name: "Барфлёр",
        type: "optional",
        place: "20 мин от Экоссвиля",
        desc: "Гранитный рыбацкий порт — «самая красивая деревня Франции». Мидии прямо с лодок. Обед у порта.",
        maps: "https://www.google.com/maps/search/?api=1&query=Barfleur+France",
        photo: "photos/barfleur.jpg",
        coordinates: [49.6734, -1.2742],
      },
      {
        time: "12:30",
        name: "Маяк Гатвиль",
        type: "optional",
        place: "Gatteville-le-Phare",
        desc: "2-й маяк Европы. 75 м, 365 ступеней, €3. Панорама.",
        maps: "https://www.google.com/maps/search/?api=1&query=Phare+de+Gatteville",
        rating: "4.7",
        photo: "photos/gatteville.jpg",
        coordinates: [49.6978, -1.2559],
      },
      {
        time: "13:30",
        name: "Устрицы Saint-Vaast",
        type: "optional",
        place: "15 мин от Барфлёра",
        desc: "Устричный порт. Дюжину у рыбаков на набережной с лимоном. Нормандия = 25% устриц Франции.",
        maps: "https://www.google.com/maps/search/?api=1&query=Saint-Vaast-la-Hougue+France",
        coordinates: [49.5886, -1.2624],
      },
      {
        time: "15:00",
        name: "Выезд в Париж",
        type: "drive",
        desc: "~3.5 ч по A84/A13. Или напрямую из Экоссвиля ~3 ч.",
      },
      {
        time: "17:30",
        name: "Отель, Париж",
        type: "sleep",
        desc: "Ночь в Париже.",
        maps: "https://www.google.com/maps/search/?api=1&query=hotels+Paris+8e",
        coordinates: [48.8738, 2.3019],
      },
      {
        time: "20:30",
        name: "Crazy Horse",
        type: "main",
        place: "12 Av. George V, Paris 8e",
        desc: "Культовое кабаре, 70 лет истории. Световые шоу + хореография. От €75/чел. Показы 20:30 и 22:45.",
        rating: "4.4",
        link: "https://www.lecrazyhorseparis.com/",
        maps: "https://www.google.com/maps/search/?api=1&query=Crazy+Horse+Paris",
        phone: "+33 1 47 23 32 32",
        booking: "Забронируй заранее!",
        coordinates: [48.8671, 2.3007],
      },
    ],
    food: [
      {
        name: "Le Safran",
        place: "Барфлёр, набережная",
        desc: "Лучший ресторан Барфлёра. Дикие мули-фрит, choucroute de la mer, St Jacques. Вид на порт. Бронируй.",
        vibe: "обед",
        price: "€20–30",
        rating: "4.5",
        maps: "https://www.google.com/maps/search/?api=1&query=Le+Safran+restaurant+Barfleur",
        phone: "+33 2 33 20 02 18",
        coordinates: [49.6735, -1.2742],
      },
      {
        name: "L'Estran",
        place: "Saint-Vaast-la-Hougue, Quai Vauban",
        desc: "4.9 рейтинг! Авторская сезонная кухня из дневного улова. Seiche, рыба дня. Напротив острова Татиу. Бронируй.",
        vibe: "обед/ужин, лучший на Котантене",
        price: "€30–45",
        rating: "4.9",
        maps: "https://www.google.com/maps/search/?api=1&query=L%27Estran+restaurant+Saint-Vaast-la-Hougue",
        phone: "+33 2 33 44 86 09",
        coordinates: [49.5886, -1.2619],
      },
      {
        name: "Café de France",
        place: "Барфлёр, набережная",
        desc: "Работает с утра до ночи. Мули-фрит, устрицы, креп. Терраса у порта, набито местными. Без брони.",
        vibe: "весь день, на ходу",
        price: "€15–25",
        rating: "4.4",
        maps: "https://www.google.com/maps/search/?api=1&query=Caf%C3%A9+de+France+Barfleur",
        phone: "+33 2 33 54 00 38",
        coordinates: [49.6734, -1.2741],
      },
      {
        name: "L'entre deux tours",
        place: "Saint-Vaast, Rue des Parcs",
        desc: "Рыбный шек — хозяин бежит к лодкам за свежим уловом. Устрицы, plateau, булоты. Без брони.",
        vibe: "морепродукты на ходу",
        price: "€20–30",
        rating: "4.8",
        maps: "https://www.google.com/maps/search/?api=1&query=L%27entre+deux+tours+Saint-Vaast-la-Hougue",
        phone: "+33 6 08 07 85 62",
        coordinates: [49.5885, -1.2621],
      },
    ],
    markets: [
      {
        name: "Maison Lesdos — устрицы",
        desc: "Устричный производитель с террасой. Дюжина + бокал мюскаде за €5–15. Булоты, копчёная форель. Без брони.",
        when: "Пн 9:30–12:30, 14–18:30",
        icon: "Shell",
        rating: "4.5",
        maps: "https://www.google.com/maps/search/?api=1&query=Maison+Lesdos+Saint-Vaast-la-Hougue",
        coordinates: [49.5886, -1.2625],
      },
      {
        name: "Порт Барфлёр",
        desc: "Мидии, крабы, булоты прямо с лодок. Подойди утром к причалу.",
        when: "Утро",
        icon: "Fish",
        maps: "https://www.google.com/maps/search/?api=1&query=Port+de+Barfleur",
        coordinates: [49.6734, -1.2742],
      },
    ],
    nearby: [
      {
        type: "photo",
        name: "Барфлёр — порт на рассвете",
        desc: "Гавань смотрит на северо-восток — восходящее солнце бьёт прямо в гранитные фасады. В 6–7 утра никого нет. Высокий прилив: дома отражаются в воде.",
        light: "Рассвет (5:30–7:00)",
        maps: "https://www.google.com/maps/search/?api=1&query=Port+de+Barfleur",
        coordinates: [49.6734, -1.2742],
      },
      {
        type: "photo",
        name: "Phare de Gatteville — скалы у маяка",
        desc: "Чёрно-оранжевый гранит разбит волнами в острые формы. При ветре брызги летят высоко. Поднимись на маяк для вида на Котантен.",
        light: "Любое время, шторм = драма",
        maps: "https://www.google.com/maps/search/?api=1&query=Phare+de+Gatteville",
        coordinates: [49.6978, -1.2559],
      },
      {
        name: "Nez de Jobourg",
        desc: "Самые высокие скалы Нормандии — 128 м. 40 мин от Барфлёра.",
        maps: "https://www.google.com/maps/search/?api=1&query=Nez+de+Jobourg",
        coordinates: [49.6878, -1.9447],
      },
    ],
  },
  {
    day: "Вторник",
    label: "Домой",
    color: "var(--stone)",
    bg: "var(--stone-light)",
    icon: "Home",
    summary: "Париж → Амстердам · ~5 ч",
    foodTip: "Круассан и кофе у отеля — Париж. Обед в Руане если заедете: La Couronne — старейший ресторан Франции (с 1345 года!), утка по-руански.",
    stops: [
      {
        time: "10:00",
        name: "Выезд из Парижа",
        type: "drive",
        desc: "Париж → Амстердам ~5 ч напрямую. Или заехать в Руан по пути (+1.5 ч).",
      },
      {
        time: "",
        name: "Руан",
        type: "optional",
        desc: "Готический собор (Моне рисовал 30 раз), фахверк, площадь Жанны д'Арк. Летом — световое шоу на фасаде.",
        maps: "https://www.google.com/maps/search/?api=1&query=Cath%C3%A9drale+Notre-Dame+de+Rouen",
        photo: "photos/rouen.jpg",
        coordinates: [49.4432, 1.0993],
      },
    ],
    food: [
      {
        name: "OKTO",
        place: "Руан, центр",
        desc: "Gault&Millau 12.5/20. Биономика из локальных продуктов, короткие цепочки. Обед: entrée+plat €26, полный €32. Открыт вт–сб.",
        vibe: "обед",
        price: "€26–32",
        rating: "4.6",
        maps: "https://www.google.com/maps/search/?api=1&query=OKTO+restaurant+Rouen",
        coordinates: [49.4428, 1.1002],
      },
      {
        name: "La Couronne",
        place: "Руан, Place du Vieux-Marché",
        desc: "Старейшая auberge Франции (1345). Canard à la Rouennaise — фирменное блюдо Руана. Историческое место у площади Жанны д'Арк.",
        vibe: "обед-легенда",
        price: "€25–50",
        rating: "4.3",
        maps: "https://www.google.com/maps/search/?api=1&query=La+Couronne+restaurant+Rouen",
        coordinates: [49.4419, 1.0938],
      },
    ],
    markets: [
      {
        name: "Marché Saint-Marc, Руан",
        desc: "Один из лучших рынков Франции. Закупись на дорогу: Neufchâtel, cidre bouché, andouille de Vire, beurre d'Isigny. Работает до ~13:00.",
        when: "Вт, Пт, Сб, Вс утро",
        icon: "Store",
        maps: "https://www.google.com/maps/search/?api=1&query=March%C3%A9+Saint-Marc+Rouen",
        coordinates: [49.4417, 1.0822],
      },
    ],
    nearby: [
      {
        type: "photo",
        name: "Rue du Gros-Horloge — до 8 утра",
        desc: "Часы 1527 года над пешеходной улицей. Встань в торце — арча обрамляет перспективу уходящих фахверков. Снимай до открытия магазинов. Мокрая брусчатка после дождя = отражения.",
        light: "Синий час / до 8:00",
        maps: "https://www.google.com/maps/search/?api=1&query=Rue+du+Gros-Horloge+Rouen",
        coordinates: [49.4427, 1.0966],
      },
      {
        type: "photo",
        name: "Côte Sainte-Catherine — панорама Руана",
        desc: "Лесистый холм восточнее центра. Шпиль собора, петля Сены, промышленные краны — всё в одном кадре. Почти никто не поднимается. Телевик сжимает слои города.",
        light: "Вечер, золотой час",
        maps: "https://www.google.com/maps/search/?api=1&query=C%C3%B4te+Sainte-Catherine+Rouen",
        coordinates: [49.4381, 1.1197],
      },
    ],
  },
];

export const DISHES = [
  {
    name: "Agneau de pré-salé AOP",
    icon: "Beef",
    where: "Воскресенье, район MSM",
    desc: "Овцы пасутся на солёных лугах залива Мон-Сен-Мишель — трава пропитана морской солью и йодом. Мясо нежное, слегка солоноватое, уникальное. Важно: именно AOP «Prés-Salés du Mont-Saint-Michel», а не просто «style pré-salé».",
  },
  {
    name: "Мули-фрит из Барфлёра",
    icon: "Shell",
    where: "Понедельник, Барфлёр",
    desc: "Барфлёр — крупнейшее месторождение диких мидий во Франции. Не фермерские, а дикие — крупнее, мясистее, с ярким морским вкусом. Сезон: июль–февраль, пик осенью.",
    photo: "photos/moules.jpg",
  },
  {
    name: "Устрицы у производителя",
    icon: "Fish",
    where: "Понедельник, Saint-Vaast",
    desc: "Нормандия = 25% всех устриц Франции. Saint-Vaast-la-Hougue — главный устричный порт. Дюжину у рыбака на набережной + лимон + бокал мюскаде. Не в ресторане с наценкой x3.",
    photo: "photos/oysters.jpg",
  },
  {
    name: "Камамбер роти",
    icon: "Flame",
    where: "Суббота, Pays d'Auge",
    desc: "Целый камамбер запекается в деревянной коробке с мёдом, орехами или травами. Макаешь хлеб. Простое блюдо, но здесь — где камамбер делают на ферме через дорогу — совсем другой вкус.",
    photo: "photos/camembert.jpg",
  },
  {
    name: "Teurgoule",
    icon: "CookingPot",
    where: "Суббота–воскресенье",
    desc: "Нормандский рисовый пудинг — рис с корицей, молоком и сахаром, запечённый 5+ часов в глиняном горшке. Корочка карамелизируется. Самое «бабушкино» блюдо Нормандии, в туристических местах не подают.",
  },
  {
    name: "Plateau de fruits de mer",
    icon: "Waves",
    where: "Понедельник, Котантен",
    desc: "Устрицы, крабы, лангустины, булоты (whelks), креветки на льду. Лучшее соотношение свежесть / цена — на побережье Котантена, прямо у порта.",
  },
  {
    name: "Tarte aux pommes normande",
    icon: "Apple",
    where: "Суббота, Pays d'Auge",
    desc: "Яблочный пирог на кальвадосе и нормандском масле. Здесь яблони за каждым забором — это их территория. Вариация: Tarte Tatin — перевёрнутый, карамельный.",
    photo: "photos/tarte.jpg",
  },
  {
    name: "Галеты и крепы",
    icon: "Wheat",
    where: "Везде",
    desc: "Нормандия / Бретань — родина. Галеты из гречневой муки: andouille de Vire + камамбер = местная классика. Крепы на десерт — с солёной карамелью и кальвадосом. Запивать болé сидра.",
  },
];

export const CHECKLIST = [
  { item: "Aéroplume", url: "https://aeroplume.fr/", critical: true },
  { item: "Via Ferrata Клеси", url: "https://vertig.fun/", critical: true },
  {
    item: "Traversée de la Baie",
    url: "https://www.cheminsdelabaie.com/",
    critical: true,
  },
  { item: "Приливы MSM", url: "https://www.maree.info/82", critical: true },
  {
    item: "Park4Night (приложение)",
    url: "https://park4night.com/",
    critical: false,
  },
  {
    item: "Domaine Dupont — проверь часы",
    url: "https://www.calvados-dupont.com/",
    critical: false,
  },
  {
    item: "La Bohème (Барфлёр) — бронь",
    url: "https://www.tripadvisor.com/Restaurant_Review-g1566276-d10169889-Reviews-La_Boheme-Barfleur.html",
    critical: false,
  },
];

export const BUDGET = [
  { item: "La Dégusterie (сыр+сидр)", cost: "€10", icon: "Utensils" },
  { item: "Domaine Dupont (бутылка)", cost: "~€5", icon: "Wine" },
  { item: "Via Ferrata + зиплайн", cost: "€35", icon: "Mountain" },
  { item: "Аббатство MSM", cost: "€16", icon: "Landmark" },
  { item: "Traversée de la Baie", cost: "~€15", icon: "Footprints" },
  { item: "Aéroplume", cost: "€75", icon: "Wind" },
  { item: "Маяк Гатвиль", cost: "€3", icon: "Lighthouse" },
  { item: "Устрицы (дюжина)", cost: "~€8", icon: "Shell" },
  { item: "Кемпинги (4 ночи)", cost: "~€60", icon: "Tent" },
  { item: "Бензин (~1 500 км)", cost: "~€120", icon: "Fuel" },
];

export const FOOD_HIGHLIGHTS =
  "камамбер роти · мули-фрит из Барфлёра · ягнёнок пре-сале · тургуль · тарт-нормандё · крепы с солёной карамелью · устрицы Saint-Vaast · поммо · кальвадос";
